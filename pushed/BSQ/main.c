...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/07/26 00:39:24 by cfrancie          #+#    #+#             */
/*   Updated: 2022/07/27 20:30:24 by cfrancie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_solution(int i, char **argv)
{
	...
	if (fd >= 0)
		...
		return ;
	ft_fill_map(map, ft_get_number_columns(argv[i]), l,
		ft_get_char_obst(argv[i]),
		ft_get_char_full(argv[i]));
	while (j < l)
	{
		ft_putstr(map[j]);
		ft_putchar('\n');
